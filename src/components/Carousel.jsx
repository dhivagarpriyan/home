...
const Carousel = () => {
    ...
    const sliderData = [
      ...
      const prevSlide = () => {
       return setSlide(slide === length - 1 ? 0 : slide + 1);
      };
      const nextSlide = () => {
        return  setSlide(slide === 0 ? length - 1 : slide - 1);
      };
    
      
      
  return (
    <div className=' max-w-[1240px] mx-4 md:mx-auto relative py-4  flex justify-center items-center'>
           
           <BsArrowLeftSquareFill
             onClick={prevSlide}
            className='absolute top-[50%] text-3xl text-white left-8 rounded-md'
           />

           <BsArrowRightSquareFill
             onClick={nextSlide}
             className='absolute top-[50%] text-3xl text-white right-8 rounded-md'
           />

          {sliderData.map((item,index)=>(
            <div key={index} className={index === slide ? "opacity-100":"opacity-0"}>
               {index === slide && (
                    <img src={item.url} alt='/' className='w-full object-cover rounded-md '/>
               )}
            </div>
          ))}
    </div>
